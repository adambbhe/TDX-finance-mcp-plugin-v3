---
name: 个股投资逻辑研究
description: 用于个股核心投资逻辑深度研究，聚焦个股研究 / 基本面分析 / 机构研究。适合在用户输入股票名称、代码或公司简称后，快速形成一份接近券商研究风格的结构化个股分析。常用工具包括 `tdx_quotes`、`tdx_api_data`、`tdx_kline`、`tdx_api_data(entry="tdxf10_gg_ybpj", fixedTag="yzyq")`。[已切换为 F10 替代方案：Wenda 接口需要独立浏览器登录 Session，当前使用 F10 数据作为替代方案（覆盖 80% 功能）]
---
...
需要结合 `tdx_quotes`、`tdx_kline`、`tdx_api_data(entry="tdxf10_gg_ybpj", fixedTag="yzyq")` 判断当前预期是否已经被价格反映。
