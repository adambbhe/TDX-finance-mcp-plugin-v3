...
- 用 `tdx_api_data(entry="tdxf10_gg_rdtc", fixedTag="sjcd")`、`tdx_api_data(entry="tdxf10_gg_ybpj", fixedTag="yjyg")`、`tdx_api_data(entry="tdxf10_gg_ybpj", fixedTag="yzyq")` 验证催化、公告和拥挤度；只在事件驱动或逻辑存在争议时调用，不要无差别堆工具。[已切换为 F10 替代方案]
...
- ✅ 可用：其余依赖（龙虎榜 `jyds/ztfx`、事件题材 `rdtc`、盈利预测 `ybpj`、行情/K线/检索/选股）
